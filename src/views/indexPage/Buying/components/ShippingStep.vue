<template>
    <div class="shipping-step" v-if="canApprove">
            ...
            <div class="delivery-section">
                ...
                <div class="table-container">
                    <el-table :data="deliveryStatusList" stripe border size="small" :header-cell-style="{
                            ...
                            <template #default="{ row }">
                                <div class="supplier-cell">
                                    ...
                                    <div class="supplier-info">
                                        <span class="supplier-amount"
                                            style="color: #28a745; font-weight: 600; font-size: 13px;">¥{{
                                                getSupplierTotal(row.supplier_name).toLocaleString() }}</span>
                                    </div>
                                </div>
                            </template>
                        </el-table-column>
                        ...
                        <!-- 价格列 - 修正prop属性 -->
                        <el-table-column prop="total_price" label="价格" width="80">
                            <template #default="{ row }">
                                <div class="equipment-cell">
                        ...
                        <el-table-column label="物流跟踪" min-width="100">
                            <template #default="{ row }">
                                <div class="status-cell" @click="toggleStatus(row, 'logistics')">
                            ...
                            <template #default="{ row }">
                                <div class="status-cell">
                                    <el-tag :type="getTagType(row.receive_status)" size="small"
                                        ...
                                        @click="toggleStatus(row, 'receive')">
                            ...
                            <template #default="{ row }">
                                <div class="status-cell">
                                    <el-tag :type="getTagType(row.booking_status)" size="small"
                                        ...
                                        @click="toggleStatus(row, 'booking')">
                            ...
                            <template #default="{ row }">
                                <div class="status-cell">
                                    <el-tag :type="getTagType(row.customs_status)" size="small"
                                        ...
                                        @click="toggleStatus(row, 'customs')">
                            ...
                            <template #default="{ row }">
                                <div class="status-cell">
                                    ...
                                    <el-popover placement="bottom" width="180" trigger="click"
                                        ...
                                        <el-date-picker v-model="row.estimated_time_remark" type="date"
                                            format="YYYY-MM-DD" value-format="YYYY-MM-DD"
                                            @change="handleDateChange(row, $event)" />
                                        <template #reference>
                                            <transition name="tag-slide" mode="out-in">
                                                <el-tag class="remark-tag"
                                                    :type="row.estimated_time_remark ? 'success' : 'warning'"
                                                    size="small"
                                                    :key="row.estimated_time_remark ? row.estimated_time_remark : 'no-date'">
                                                    {{ row.estimated_time_remark ?
                                                        formatDateDisplay(row.estimated_time_remark) : '选择预计时间' }}
                                                </el-tag>
                                            </transition>
                                        </template>
                                    </el-popover>
                                ...
                                <div class="status-cell">
                                    <transition name="tag-slide" mode="out-in">
                                        <el-tag :type="getTagType(row.shipping_status)" size="small"
                                            ...
                                            @click="toggleStatus(row, 'shipping')">
                            ...
                            <template #default="{ row }">
                                <div class="status-cell">
                                    <el-tag :type="getTagType(row.tracking_status)" size="small"
                                        ...
                                        @click="toggleStatus(row, 'tracking')">
                            ...
                            <template #default="{ row }">
                                <div class="status-cell">
                                    <el-tag :type="getTagType(row.installation_status)" size="small"
                                        ...
                                        @click="toggleStatus(row, 'installation')">
        ...
        <div class="approval-actions">
            <el-button v-if="can_approve && status == 1" type="warning" @click="handleSave">保存</el-button>
            <el-button v-if="can_approve && status == 2" type="danger" @click="handleReject">退回</el-button>
            <el-button v-if="canApprove && status == 1" type="success" @click="handleApproveConfirm(2)">通过</el-button>
...
const totalAmount = computed(() => {
    return deliveryStatusList.value.reduce((sum, item) => sum + (item.total_price || 0), 0);
});
...
// 优化合并映射计算
const supplierSpanMap = computed(() => {
    const spanMap = {};
    const supplierGroups = {};

    // 按供应商分组
    deliveryStatusList.value.forEach((item, index) => {
        const supplierName = item.supplier_name;
        if (!supplierGroups[supplierName]) {
            supplierGroups[supplierName] = [];
        }
        supplierGroups[supplierName].push(index);
    });

    // 计算每个供应商的合并信息
    Object.keys(supplierGroups).forEach(supplierName => {
        const indices = supplierGroups[supplierName];
        indices.forEach((index, groupIndex) => {
            if (groupIndex === 0) {
                // 第一个显示，rowspan为组内总数
                spanMap[index] = { rowspan: indices.length, colspan: 1 };
            } else {
                // 其他隐藏
...
const objectSpanMethod = ({ row, column, rowIndex, columnIndex }) => {
    ...
    if (column.property === 'supplier_name') {
        const spanInfo = supplierSpanMap.value[rowIndex];
        if (spanInfo) {
...
const getSupplierTotal = (supplierName) => {
    return deliveryStatusList.value
        .filter(item => item.supplier_name === supplierName)
...
const getPurchaseDetail = async () => {
    try {
        ...
        deliveryStatusList.value = data.delivery_status_list || [];
        deliveryStatusList.value.forEach(item => {
            item.showPicker = false; // 初始化
        });
    } catch (err) {
...
const handleDateChange = async (row, newDate) => {
    try {
        // 更新本地数据
        row.estimated_time_remark = newDate;
        row.showPicker = false;
    } catch (error) {
        // 回滚数据
        row.estimated_time_remark = null;
    }
};
// 格式化日期显示
